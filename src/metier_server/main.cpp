...
auto main(int argc, char* argv[]) -> int
{
    ...
    try {
        ...
        auto opts = metier_server::Options{argc, argv, alloc};
        ...
        if (opts.start_sync_server_) {
            ...
            auto const started = client.Network().OTDHT().StartListener(
                opentxs::String{prefix}.append(internal).append(sep).append(
                    std::to_string(port)),
                opentxs::String{prefix}
                    .append(opts.sync_server_public_ip_)
                    .append(sep)
